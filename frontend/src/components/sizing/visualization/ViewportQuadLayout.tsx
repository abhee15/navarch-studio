...
import type { VisualizationOptions } from "../workspace/VisualizationSettings";
import {
  exportSVG,
  exportSVGToPNG,
  exportCanvasToPNG,
...
interface ViewportQuadLayoutProps {
  ...
  vizSettings?: VisualizationOptions;
}
 ...
 */
export const ViewportQuadLayout: React.FC<ViewportQuadLayoutProps> = observer(
  ({ candidate, vizSettings }) => {
    const [mode, setMode] = useState<ViewportMode>("quad");

    // Use settings from parent or defaults
    const show3DWaterplane = vizSettings?.show3DWaterplane ?? true;
    const show3DCenters = vizSettings?.show3DCenters ?? true;
    const show3DLabels = vizSettings?.show3DLabels ?? true;
    const show3DGrid = vizSettings?.show3DGrid ?? true;
    const show3DWaterlines = vizSettings?.show3DWaterlines ?? false;
    const show3DButtocks = vizSettings?.show3DButtocks ?? false;
    const show3DSections = vizSettings?.show3DSections ?? false;
