...
export const Step2HullFamilies: React.FC<Step2Props> = ({
  ...
  const handleMaskVersionChange = (value: string) => {
    const parsed = value ? parseInt(value, 10) : undefined;
    updateFormData({ familyMaskVersion: Number.isNaN(parsed) ? undefined : parsed });
  };
  ...
  const isValid =
    ...
    !!formData.midshipFamily &&
    !!formData.sternFamily &&
    (!!formData.familyMaskVersion || !taxonomyEntry);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        <div className="space-y-2">
          <Label htmlFor="familyMaskVersion">Family Mask Version</Label>
          <Input
            id="familyMaskVersion"
            type="number"
            min={1}
            value={formData.familyMaskVersion?.toString() || ""}
            onChange={(e) => handleMaskVersionChange(e.target.value)}
          />
          <p className="text-xs text-gray-500 dark:text-gray-400">
            Keeps track of taxonomy revisions. Default provided by metadata (usually 1).
          </p>
        </div>

        <div className="space-y-2">
          <Label htmlFor="shipdInputVectorJson">Custom ShipD Vector (optional)</Label>
          <textarea
