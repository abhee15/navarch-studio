 ...
 */
export function useWorkspaceLayout(vesselId: string) {
  const storageKey = `${STORAGE_KEY_PREFIX}-${vesselId}`;
  ...
  const loadLayout = useCallback((): WorkspaceLayout => {
    ...
    return {
      mode: "edit", // Start in edit mode (will be overridden by smart logic)
  ...
  const togglePanelCollapsed = useCallback((panelId: PanelId) => {
    setLayout((prev) => ({
      ...
      panelStates: {
        ...
        [panelId]: {
          ...prev.panelStates[panelId],
          collapsed: !prev.panelStates[panelId].collapsed,
        },
      },
      lastModified: new Date().toISOString(),
    }));
  ...
  const togglePanelHidden = useCallback((panelId: PanelId) => {
    setLayout((prev) => ({
      ...
      panelStates: {
        ...
        [panelId]: {
          ...prev.panelStates[panelId],
          hidden: !prev.panelStates[panelId].hidden,
        },
      },
      lastModified: new Date().toISOString(),
    }));
