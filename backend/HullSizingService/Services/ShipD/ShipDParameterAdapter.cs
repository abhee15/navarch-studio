...
public class ShipDParameterAdapter : IShipDParameterAdapter
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // INTERNAL USE ONLY: ShipD vector override support
        // This allows internal/testing tools to provide raw parameter vectors.
        // Regular users should use vessel type selections and geometry parameters instead.
        if (!string.IsNullOrWhiteSpace(runRequest.ShipdInputVectorJson))
        {
            try
            {
                ...
                if (overrideVector != null && overrideVector.Count == ShipDParameterCount)
                {
                    ...
                    _logger.LogInformation("[SHIPD_ADAPTER] Using provided ShipD vector override (internal use)");
                }
                else
                {
            ...
            catch (JsonException ex)
            {
                ...
                _logger.LogWarning(ex, "[SHIPD_ADAPTER] Failed to parse ShipD vector override. Falling back to autogenerated defaults.");
            }
        }
