...
/// <summary>
/// Controller that proxies catalog requests to the DataService
/// </summary>
[ApiController]
...
public class CatalogController : ControllerBase
{
    ...
    public async Task<IActionResult> GetProxy(string path, CancellationToken cancellationToken)
    {
        try
        {
            _logger.LogInformation("Proxying GET /catalog/{Path} to DataService", path);
            var response = await _httpClientService.GetAsync("data", $"api/v1/catalog/{path}", cancellationToken);
    ...
    public async Task<IActionResult> PostProxy(string path, CancellationToken cancellationToken)
    {
        try
        {
            _logger.LogInformation("Proxying POST /catalog/{Path} to DataService", path);
            ...
            if (!string.IsNullOrEmpty(contentType) && contentType.StartsWith("multipart/form-data", StringComparison.OrdinalIgnoreCase))
            {
                ...
                var response = await _httpClientService.PostAsync("data", $"api/v1/catalog/{path}", multipartContent, cancellationToken);
            ...
            else
            {
                ...
                var response = await _httpClientService.PostAsync("data", $"api/v1/catalog/{path}", content, cancellationToken);
                var responseContent = await response.Content.ReadAsStringAsync(cancellationToken);
    ...
    public async Task<IActionResult> PutProxy(string path, CancellationToken cancellationToken)
    {
        try
        {
            _logger.LogInformation("Proxying PUT /catalog/{Path} to DataService", path);
            ...
            var response = await _httpClientService.PutAsync("data", $"api/v1/catalog/{path}", content, cancellationToken);
            var responseContent = await response.Content.ReadAsStringAsync(cancellationToken);
    ...
    public async Task<IActionResult> DeleteProxy(string path, CancellationToken cancellationToken)
    {
        try
        {
            _logger.LogInformation("Proxying DELETE /catalog/{Path} to DataService", path);
            var response = await _httpClientService.DeleteAsync("data", $"api/v1/catalog/{path}", cancellationToken);
            var content = await response.Content.ReadAsStringAsync(cancellationToken);
